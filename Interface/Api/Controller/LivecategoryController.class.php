...
//    }
    ...
    //20161205李卫星 21：40 PC版分析师单独房间版本更新以后接口也需要更新
	public function allwebcast()
    {
		//\Predis\Autoloader::register();
        //$redis = new \Predis\Client();
        ...
        if($userflag){
			...
			if($pageIndex=="1"){
				...
				//或者分页读取
				$pageCount=20*($pageIndex-1);
				$recomsult=M()->query('SELECT u.userid,r.roomid,r.roomtitle,r.speaker,r.hits,r.roomcode,r.roomnumber,r.title,r.webcastkeyid,r.wsurl,r.rtmpurl,r.tlabel,r.status,r.headimage as roomimage,r.isonly,r.isrtype,r.classifyid,u.nickname,u.headimage,r.classifyid,c.classifyname FROM wht_rooms AS r LEFT JOIN wht_userinfos AS u ON r.roomid = u.roomid LEFT JOIN wht_classify AS c ON c.classifyid = r.classifyid where r.isdelete=0 and u.isdelete=0 and u.usergrade=1 and u.usertype=2 and r.isonly=0 and r.status in(1,2)  and r.shenhe=1  and r.classifyid='.$type.' order by status asc LIMIT '.$pageCount.',20');
				$result = array();
				foreach ($recomsult as $key => $value) {
					if($value['wsurl']){
						$wsurls = 'ws://'.$value['wsurl'];
