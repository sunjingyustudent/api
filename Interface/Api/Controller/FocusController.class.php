...
class FocusController extends ApiController {
    ...
    public function focus() {
        ...
        if (!empty($getfocus)) {
            ...
            $roomid = $getfocus["roomid"];
            
            if ($this->checktoken($atoken)) {
                $where = array(array("roomid"=>$roomid));
                //$where["status"] = 1;
                $where["isdelete"] = 0;
                if(D("rooms")->where($where)->count() <= 0){
                    $this->myApiPrint('没有该房间信息', '300');
                }
                $userrooms = D('userrooms');
                $redis = $this->connectredis();
                $user = $redis->hgetall($atoken);
                $wh = array();
                $wh["roomid"] = $roomid;
                $wh["userid"] = $user["uid"];
                if($userrooms->where($wh)->count() > 0){
                    $this->myApiPrint('不能重复关注', '300');
                }
                    
                $result = $userrooms->add($wh);
                if ($result) {
                    $this->myApiPrint('关注成功', '200');
                }
                $this->myApiPrint('关注失败', '300');
            }
            $this->myApiPrint('accesstoken don\'t find', '404');
        }
        
        $this->myApiPrint('网络繁忙', '300');
    }
    ...
    /*public function usermarket() {
        ...
        if (!empty($token)) {
            ...
            if ($user) {
                ...
                $userid = $userid["uid"];
                $result1 = M("rooms r")
                        ->where('r.isdelete = 0 AND ui.usergrade = 1 AND r.roomid in(select roomid from wht_userrooms where userid=' . $userid . ')')
            ...
            if ($user) {
                ...
                foreach ($result1 as $key => $vv) {
                    ...
                    $result[$key] = array(
                        ...
                        "roomid" => $vv['roomid'],
                        "roomcode" => $vv['roomcode'],
                        "roomtitle"=>$vv['nickname'],//app上显示的标题
                        "webcastkeyid" => $vv['webcastkeyid'],
                        "headimage" => $vv['headimage'],
                        "classifyid" => $vv['classifyid'],
                        "hits" => $vv['hits'],
