...
//            }
	...
	public function androidequipmenttest()
    {
        $rm = I("get.");
        $uuid = $rm['uuid'];
        ...
        if($userflag){
			...
			if (!empty($uuid) && !empty($smartphones)){
				$owner = M('equipmentfile');
				$where1['uuid']=$uuid;//判断uuid是否存在
				$resn = $owner->where($where1)->field("autoid,emcid,emcpwd")->find();
				if(!empty($resn)){
					//存在需要判断emcid和emcpwd是否为空，如果为空，那么注册一个帐号并且修改数据，不为空，直接显示查询出来的字段
					if(empty($resn['emcid'])&&empty($resn['emcpwd'])){
						//注册信息到环信上
						$where2["emcid"]=randCode(3).uniqid();
						$where2["emcpwd"]=md5('123');
						vendor('emchat.Easemob');//调用环信的class类进行注册用户
						$options['client_id']='YXA6Nt1u0HosEeaqHH3gPQ64JQ';
						$options['client_secret']='YXA6zcifjTMaxc6_nIme-vJyanwhX6o';
						$options['org_name']='haoontech888';
						...
						$aa1 = $h->getToken();
						$rresult = "Authorization:Bearer ".$aa1['access_token'];
						$arrResult = $h->createUser($where2["emcid"],$where2["emcpwd"],$rresult);
						//注册或删除失败时，uuid为空。根据uuid是否有值 判断操作是否成功
						$result = $arrResult['entities'][0]['uuid']?$arrResult['entities'][0]['uuid']:"";
						if(empty($result)){
							//注册失败，重新再注册，再失败则修改uuid的信息
							$aa = $h->createUser($where2["emcid"],$where2["emcpwd"],$rresult);
							$result3 = $aa['entities'][0]['uuid']?$aa['entities'][0]['uuid']:"";
							if(empty($result3)){
								//修改信息，什么都不返回
								$owner->where(array("autoid"=>$resn['autoid']))->save(array("emcid"=>'',"emcpwd"=>''));
								$this->myApiPrint('success', 200);
							}else{
								//修改信息，返回刚刚添加的数据
								$owner->where(array("autoid"=>$resn['autoid']))->save(array("emcid"=>$where2["emcid"],"emcpwd"=>$where2["emcpwd"]));
								$result5 = array(
									"emcid"=>$where2["emcid"],
									"emcpwd"=>$where2["emcpwd"],
									"token"=>$token
								);
								$this->myApiPrint('success', 200,$result5);
							}
						}else{
							//返回查询出来的数据
								$result5 = array(
					...
					if ($count>0) {
						...
						if(empty($result)){
							...
							if(empty($result3)){
								//修改信息，什么都不返回
								$owner->where(array("autoid"=>$count))->save(array("emcid"=>'',"emcpwd"=>''));
								$this->myApiPrint('success', 200);
							}else{
							    //添加成功，返回
								$result5 = array(
